...
from saap.crispy_layout_mixin import CrispyLayoutFormMixin, get_field_display, SaapFormHelper
from saap.utils import normalize

logger = logging.getLogger(settings.BASE_DIR.name)

LIST, DETAIL, ADD, CHANGE, DELETE, CREATE, UPDATE = \
    '.list_', '.detail_', '.add_', '.change_', '.delete_', '.add_', '.update_'
...
class PermissionRequiredForAppCrudSaapMixin(PermissionRequiredMixin):
...
class CrudSaapBaseMixin(CrispyLayoutFormMixin):

    def __init__(self, **kwargs):
        super(CrudSaapBaseMixin, self).__init__(**kwargs)
    ...
    def get_template_names(self):
        names = super(CrudSaapBaseMixin, self).get_template_names()
...
class CrudSaapListView(PermissionRequiredContainerCrudSaapMixin, ListView):
...
class CrudSaapCreateView(PermissionRequiredContainerCrudSaapMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        return super(CrudSaapCreateView, self).get_context_data(**kwargs)
...
class CrudSaapDetailView(PermissionRequiredContainerCrudSaapMixin,
    ...
    def get_headers(self):
        ...
        try:
            ...
            self.logger.error("user=" + email + ". " + str(e))
            obj = self.crud if hasattr(self, 'crud') else self
            return [getattr(
                self.object,
...
class CrudSaapUpdateView(PermissionRequiredContainerCrudSaapMixin,
...
class CrudSaapDeleteView(PermissionRequiredContainerCrudSaapMixin,
    ...
    def delete(self, request, *args, **kwargs):
        try:
            return super(CrudSaapDeleteView, self).delete(request, args, kwargs)
...
class CrudSaap:
    BaseMixin = CrudSaapBaseMixin
    ListView = CrudSaapListView
    CreateView = CrudSaapCreateView
    DetailView = CrudSaapDetailView
    UpdateView = CrudSaapUpdateView
    DeleteView = CrudSaapDeleteView
    help_topic = ''
    ...
    def get_urls(cls):
        ...
        cruds_base = [
            (CrudSaapListView.get_url_regex()
             if CrudSaapListView else None, CrudSaapListView, ACTION_LIST),
            (CrudSaapCreateView.get_url_regex()
             if CrudSaapCreateView else None, CrudSaapCreateView, ACTION_CREATE),
            (CrudSaapDetailView.get_url_regex()
             if CrudSaapDetailView else None, CrudSaapDetailView, ACTION_DETAIL),
            (CrudSaapUpdateView.get_url_regex()
             if CrudSaapUpdateView else None, CrudSaapUpdateView, ACTION_UPDATE),
            (CrudSaapDeleteView.get_url_regex()
             if CrudSaapDeleteView else None, CrudSaapDeleteView, ACTION_DELETE)]
    ...
    def build(cls, _model, _help_topic, _model_set=None, list_field_names=[]):
        ...
        ModelCrudSaap = create_class(list_field_names)
        ModelCrudSaap.__name__ = '%sCrud' % _model.__name__
        return ModelCrudSaap


class CrudSaapAux(CrudSaap):
    """
    ...
    def build(cls, _model, _help_topic, _model_set=None, list_field_names=[]):
        ...
        ModelCrudSaap = Crud.build(
            _model, _help_topic, _model_set, list_field_names)

        class ModelCrudSaapAux(CrudAux, ModelCrud):
            pass

        return ModelCrudSaapAux


class MasterDetailCrudSaap(CrudSaap):
    is_m2m = False
    ...
    class ListView(CrudSaap.ListView):
        ...
        def get(self, request, *args, **kwargs):
            return CrudSaap.ListView.get(self, request, *args, **kwargs)
    ...
    class CreateView(CrudSaap.CreateView):
        ...
        def get_form(self, form_class=None):
            obj = self.crud if hasattr(self, 'crud') else self
            form = super(MasterDetailCrudSaap.CreateView, self).get_form(
                self.form_class)
        ...
        def get_context_data(self, **kwargs):
            ...
            obj = self.crud if hasattr(self, 'crud') else self
            context = CrudSaap.CreateView.get_context_data(
                self, **kwargs)
    ...
    def build(cls, model, parent_field, help_topic,
        ...
        crud = super(MasterDetailCrudSaap, cls).build(
...
class CrudSaapBaseForListAndDetailExternalAppView(MasterDetailCrudSaap):
    CreateView, UpdateView, DeleteView = None, None, None

    class BaseMixin(CrudSaap.PublicMixin, MasterDetailCrudSaap.BaseMixin):

        def resolve_url(self, suffix, args=None):
            obj = self.crud if hasattr(self, 'crud') else self

            """ namespace deve ser redirecionado para app local pois
            o models colocados nos cruds que herdam este CrudSaap são de outras app
            """
