...
from saap.crud.base_saap import CrudSaap, CrudSaapBaseMixin, CrudSaapListView, CrudSaapCreateView,\
    CrudSaapUpdateView, CrudSaapDeleteView, CrudSaapDetailView, make_pagination

from saap.cerimonial.forms import PerfilForm
from saap.cerimonial.models import Perfil
from saap.core.models import OperadorAreaTrabalho
...
class PermissionRequiredContainerCrudSaapMixin(PermissionRequiredMixin):
...
class DetailMasterCrudSaap(CrudSaap):

    class BaseMixin(CrudSaapBaseMixin):
    ...
    class ListView(
            PermissionRequiredContainerCrudSaapMixin, CrudSaapListView):
        ...
        def get_queryset(self):
            queryset = CrudSaapListView.get_queryset(self)
    ...
    class CreateView(
            PermissionRequiredContainerCrudSaapMixin, CrudSaapCreateView):
    ...
    class UpdateView(
            PermissionRequiredContainerCrudSaapMixin, CrudSaapUpdateView):
    ...
    class DeleteView(
            PermissionRequiredContainerCrudSaapMixin, CrudSaapDeleteView):
        permission_required = (DELETE, )

    class DetailView(
            PermissionRequiredContainerCrudSaapMixin,
            CrudSaapDetailView, MultipleObjectMixin):
    ...
    def build(cls, _model, _model_set, _help_path):
        ...
        class ModelCrudSaap(cls):
            model = _model
            model_set = _model_set
            help_path = _help_path

        ModelCrudSaap.__name__ = '%sCrud' % _model.__name__
        return ModelCrudSaap


class MasterDetailCrudSaapPermission(CrudSaap):
    is_m2m = False
    ...
    class BaseMixin(CrudSaap.BaseMixin):
        ...
        def get_context_data(self, **kwargs):
            ...
            context = super(CrudSaapBaseMixin, self).get_context_data(**kwargs)
    ...
    class ListView(CrudSaap.ListView):
        ...
        def get(self, request, *args, **kwargs):
            response = DetailMasterCrudSaap.ListView.get(
                self, request, *args, **kwargs)
        ...
        def get_context_data(self, **kwargs):
            obj = self.crud if hasattr(self, 'crud') else self
            count = self.object_list.count()
            context = CrudSaapListView.get_context_data(self, **kwargs)
            context['count'] = count
    ...
    class CreateView(CrudSaap.CreateView):
        ...
        def get_context_data(self, **kwargs):
            obj = self.crud if hasattr(self, 'crud') else self
            context = DetailMasterCrudSaap.CreateView.get_context_data(
                self, **kwargs)
    ...
    def build(cls, model, parent_field, help_path):
        crud = super(MasterDetailCrudSaapPermission, cls).build(model, help_path)
        crud.parent_field = parent_field
        return crud


class PerfilAbstractCrudSaap(DetailMasterCrudSaap):
    model_set = None
    model = Perfil
    ...
    class DetailView(DetailMasterCrudSaap.DetailView):
        ...
        def get(self, request, *args, **kwargs):
            ...
            try:
                ...
                return redirect(reverse('saap.cerimonial:perfil_create'))
    ...
    class UpdateView(DetailMasterCrudSaap.UpdateView):
        ...
        def post(self, request, *args, **kwargs):
            ...
            return DetailMasterCrudSaap.UpdateView.post(self, request, *args, **kwargs)

    class DeleteView(DetailMasterCrudSaap.DeleteView):
        ...
        def get(self, request, *args, **kwargs):
            if request.user.is_authenticated():
                if request.user.contato_set.exists():
                    return redirect(reverse('saap.cerimonial:perfil_detail'))

            return DetailMasterCrudSaap.CreateView.get(self, request, *args, **kwargs)

        def form_valid(self, form):
            fv = DetailMasterCrudSaap.CreateView.form_valid(self, form)
    ...
    def get_urls(cls):
        def _add_base(view):
            class CrudSaapViewWithBase(cls.BaseMixin, view):
                model = cls.model
                help_path = cls.help_path
                crud = cls
            CrudSaapViewWithBase.__name__ = view.__name__
            return CrudSaapViewWithBase

        CrudSaapCreateView = _add_base(cls.CreateView)
        CrudSaapDetailView = _add_base(cls.DetailView)
        CrudSaapUpdateView = _add_base(cls.UpdateView)
        CrudSaapDeleteView = _add_base(cls.DeleteView)

        return [
            url(regex, view.as_view(), name=view.url_name(suffix))
            for regex, view, suffix in [
                (CrudSaapListView.get_url_regex(), CrudSaapDetailView, base.DETAIL),
                (CrudSaapCreateView.get_url_regex(), CrudSaapCreateView, base.CREATE),
                (r'^edit$', CrudSaapUpdateView, base.UPDATE),
                (r'^delete$', CrudSaapDeleteView, base.DELETE),
            ]]


class PerfilDetailCrudSaapPermission(MasterDetailCrudSaapPermission):

    class BaseMixin(MasterDetailCrudSaapPermission.BaseMixin):
        ...
        def get_context_data(self, **kwargs):
            ...
            context = super(MasterDetailCrudSaapPermission.BaseMixin,
                            self).get_context_data(**kwargs)

            context['subnav_template_name'] = 'cerimonial/subnav_perfil.yaml'
            return context

    class ListView(MasterDetailCrudSaapPermission.ListView):
        ...
        def get_queryset(self):
            qs = super(CrudSaapListView, self).get_queryset()
    ...
    class CreateView(MasterDetailCrudSaapPermission.CreateView):
    ...
    class DeleteView(MasterDetailCrudSaapPermission.DeleteView):

        def get_success_url(self):
            return self.resolve_url(base.LIST)

    class DetailView(MasterDetailCrudSaapPermission.DetailView):
    ...
    def get_urls(cls):
        ...
        CrudSaapListView = _add_base(cls.ListView)
        CrudSaapCreateView = _add_base(cls.CreateView)
        CrudSaapDetailView = _add_base(cls.DetailView)
        CrudSaapUpdateView = _add_base(cls.UpdateView)
        CrudSaapDeleteView = _add_base(cls.DeleteView)
        ...
        return [
            url(regex, view.as_view(), name=view.url_name(suffix))
            for regex, view, suffix in [
                (CrudSaapListView.get_url_regex(),
                 CrudSaapListView, base.LIST),
                (CrudSaapCreateView.get_url_regex(),
                 CrudSaapCreateView, base.CREATE),
                (CrudSaapDetailView.get_url_regex(),
                 CrudSaapDetailView, base.DETAIL),
                (CrudSaapUpdateView.get_url_regex(),
                 CrudSaapUpdateView, base.UPDATE),
                (CrudSaapDeleteView.get_url_regex(),
                 CrudSaapDeleteView, base.DELETE),
            ]]
